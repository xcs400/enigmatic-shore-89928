...
//		myWstream1.write ( t);
	...
	//	console.log("tunnel.js: write back to real stream ["+b+ "]")
 	//	console.log("tunnel.js: write back to real stream   len=" , curRealStream._restLenOfDataToRead )
 	//	console.log(b)
  	//	console.log("tunnel.js: write back to real stream ")
 // myWstream1.write("<br>\r\n \r\n ---------block ------------ \r\n<br>")
...
function handle_tunnel_event(tunnel, args) {
 ...
 // if (eventName  &&  streamId[0] == 'F' )
//		console.log("streamId:" + streamId.slice(0,16) + " eventName:{"+ eventName.slice(0,2) +"}" )
...
// console.log(tunnel._tag + 'invalid event :' +eventName);
...
function pipe_stream_to_tunnel(tunnel, realStream, streamId) {
  realStream
   ...
   //   tunnel.cork();
	...
	console.log(`\r\n\r\n*** ${tunnel._tag}[${streamId}]  Write data back to tunel lg:` + buf.length + "->" + buf1);
	  ...
	  var bufin= Buffer.from(buf,'binary')
 ...
 // myWstream1.write(buf)
 	...
 	//  var buf1=`${streamId}\t:\t${buf.length.toString(16)}\n`+buf
 
   //   tunnel.write(`${streamId}\t:\t${buf.length.toString(16)}\t${inc}\n`);
      tunnel.write(buf3);
 //     tunnel.uncork();
    })
    .on('end', () => {
